...
## Differences from other Pokémon tabletop systems

There are 3 unavoidable issues with Pokémon tabletops:

1. Video game faithfulness makes bad tabletops, faithfulness to tabletop conventions makes a bad Pokémon game. Compromise.
2. They’re complicated: there's so many monsters. Lessen additional complexity.
3. With this many moving parts, it’ll break *somewhere*. Game Master (GM) judgment is the only solution.

The best simplification is to *remove things*. The next is to *not add things*. We tried other tricks to manage complexity where we couldn't uproot it:

<dl>
<dt>Reuse existing knowledge.
  <dd>If it can be like the video games, it is. Who’s playing that’s not a big ol’ Pokémon nerd?
But if we can also match tabletop tropes, we do.
<dt>Simple trunk, complex leaves.
  <dd>Individual rules can be complex if that makes the main system simpler. This reduces the knowledge everyone must have.
<dt>Avoid front-loaded decisions.
  <dd>Classes are fewer but deeper, and without prerequisites. New systems are intimidating. If we don't welcome new players, we’ll have <b><i>no</i></b> players. (Experienced players can start at higher levels.)
</dl>
...
- **PP works differently and is renamed to AP to reflect that.** As a result, PP-related things like Spite, PP Ups, Pressure, etc. have also changed.
...
- Move changes:
  ...
  + **Mirror Move (“Parrot Mimicry” in Japanese) is now a Sonic move**, mostly to combo with Punk Rock and other things that care about that.
...
- Annoying evolution methods gained alternatives:
  + **_All_ Eeveelutions can also evolve from an evolutionary stone.**
  + **_All_ evolutions from strong magnetic fields can also use Thunder Stones.**
  + **Trade evolutions can also evolve by level** or using up their trade items.

- **Some evolution items gained held-item effects.**
...
- Egg Groups with names confusable with Types got renamed to disambiguate:
  + Water 1 → Aquatic
  + Water 3 → Crustacean (Yeah, we know about Archen. Better name suggestions welcome)
  + Grass → Plant
  + Bug → Insect
  + Fairy → Pixie
  + etc.
